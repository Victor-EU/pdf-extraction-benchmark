...
| **Document-AI parser** | Landing AI ADE, **Mistral OCR**, **Pulse / runpulse.com**, LlamaParse, Azure Document Intelligence, AWS Textract, Google Document AI | **Exact bounding boxes / provenance**, table structure, scale, in-tenant deployment; **their top "agentic"/LVM tiers now read charts & diagrams at near-vision quality** | Per-page cost; **basic/default tiers trail badly on figures and can silently drop pages** — you must select the most capable tier; **and that capable tier can over-assert or even *fabricate* figure prose** (Mistral OCR 4's advanced chart-annotation layer reads charts yet invents content on graphics it can't parse — 19% unsupported, the highest in this benchmark). **But fidelity varies *enormously within this class*:** Pulse (Ultra 2), at the same advanced-tier setting, was the **lowest-fabrication vision tool in the whole benchmark (5% unsupported)** at 86% capture — the trade was latency (`refine` ~19 s/pg), not invention. So don't treat "Document-AI" as one fidelity profile; surface confidence scores, verify figure-derived claims, and **measure fabrication per vendor** before you trust the tier. | You need coordinates, auditability, compliance, fixed forms |
