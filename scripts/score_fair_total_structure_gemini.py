#!/usr/bin/env python3
"""Cross-family STRUCTURE-AWARE fair total — Gemini judge.

Twin of score_fair_total_structure.py: imports the BYTE-IDENTICAL structure-aware PROMPT_HEAD and
16k caps, so the only variable vs the gpt-5 structure judge is the judge model family. Output:
results/_fair_total_judging_gemini_v2.json (the NEW canonical Gemini judging; the content-rubric
version is preserved at results/_fair_total_judging_gemini_v2_content.json).
"""
import os, sys, json, time, random, threading
from concurrent.futures import ThreadPoolExecutor
import requests, certifi
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from score_fair_total_structure import PROMPT_HEAD, GT_CAP, VEND_CAP, LETTERS
...
def main():
    ...
    cache_dir = os.environ.get("FT_CACHE", "ground_truth/fair_total_structure_gemini")
    ...
    print(f"GEMINI structure-aware fair total: {len(targets)} pages -> {out_path}")
    ...
    print(f"GEMINI structure judge ({MODEL}): judged {len(out)}; errors {len(errs)}; "
