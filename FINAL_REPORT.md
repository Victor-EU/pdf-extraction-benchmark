...
| 5 | **Mistral OCR 4** (advanced) | **80%** | 84% | −4 | **19%** | ~$3 ($5/1k) | **exact boxes** |
| 6 | PyMuPDF | **68%** | 84% | **−16** | 5% | **$0** usage¹ | exact boxes |
| 7 | LiteParse | **62%** | 80% | **−18** | 8% | $0 (local) | exact boxes |
| 8 | Tesseract | **52%** | 64% | **−12** | 15% | $0 | word boxes |
| ◆ | gpt-5 (image) | 88% ◆ | 91% | −3 | 9% | $13.82 | coarse |
| ◆ | gpt-5 (file) | 87% ◆ | 91% | −4 | 9% | $12.54 | coarse |

> **Scores are structure-aware** (binding-aware rubric, both judge families). Landing AI is the current DPT-2 model; all ten vendors were judged with byte-identical prompts and shuffles. Three additions were validated as controlled changes — the DPT-2 re-judge as a swap (every other vendor's aggregate moved <1.1pp; [`LANDINGAI_DPT2_REBENCH.md`](LANDINGAI_DPT2_REBENCH.md)), **LiteParse as an add** (re-judged 9-up; the existing eight moved <1.34pp; [`LITEPARSE_ADD.md`](LITEPARSE_ADD.md)) and **Mistral OCR 4 as an add** (re-judged 10-up with weights frozen to canonical; the existing nine moved <1.41pp across all four judge passes; [`MISTRAL_ADD.md`](MISTRAL_ADD.md)). The structure change is identical across judges (PyMuPDF −16 under *both* gpt-5 and Gemini). Ranking order: Gemini Flash clear #1; a four-way 86–89 high-quality cluster; **Mistral OCR 4 a vision-tier standalone at 80% — but with the highest unsupported rate of any vendor (19%), because its advanced figure-annotation layer fabricates on graphics it can't read (see [`MISTRAL_ADD.md`](MISTRAL_ADD.md))**; the three local text-layer tools fall to a separate class. The content-recall numbers (graded against the corrected v2 `GROUND_TRUTH.md`; see §9) are preserved in `results/_fair_total_judging_content.json`.
>
