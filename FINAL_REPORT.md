...
> **Operational note — speed.** Per-page extraction latency spans **~195×**: PyMuPDF **0.11 s/page** (local) to Landing AI **21.5 s/page** (DPT-2, page-by-page harness). The cloud *LLM* vision tier clusters at **4–7 s/page** (Gemini Flash-Lite 4.4, gpt-5-image 4.1, Gemini Flash 6.9, gpt-5-file 7.0); the two slow ends are **document-AI parsers** — Landing AI (21.5, a harness artifact) and **Pulse (18.8, the genuine cost of its `refine` re-OCR pass)**. Latency does *not* track capture on this corpus (fastest = weakest). Full table and the measurement caveats are in §6.
